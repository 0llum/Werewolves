...
public class Chatroom extends AppCompatActivity implements RoomRequestListener, NotifyListener{
        ...
        if(event.getResult()==0){
            if(event.getJoinedUsers().length>1){// if more than one user is online
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Utils.showToast(Chatroom.this, "No online user found");
                    }
                });
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Utils.showToast(Chatroom.this, "Error in fetching data. Please try later");
                }
            });
        }
    }
